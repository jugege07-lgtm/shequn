...
export class PaymentService {
  ...
  async createUnifiedOrder(params: {
    ...
    const { orderNo, amount, description, openid, clientIp, payType } = params;

    // 判断支付方式：
    // 1. 前端显式传 payType 时以它为准
    // 2. 未传时自动判断：真实微信 openid（小程序 wx.login 获取）→ JSAPI；手机号/模拟 openid → H5 支付
    const isRealOpenid = !!openid && !openid.startsWith('mock_') && !openid.startsWith('phone_') && !openid.startsWith('admin_');
    const isH5 = payType ? payType === 'h5' : !isRealOpenid;

    if (!isH5) {
      // JSAPI 模式必须有真实 openid
      if (!openid) {
        throw new BadRequestException('缺少用户 openid，无法发起微信支付');
      }
      if (!isRealOpenid) {
        throw new BadRequestException('当前用户未通过微信授权，无法发起微信支付');
      }
    } else {
      // H5 模式不需要 openid，仅记录日志
      if (openid && !isRealOpenid) {
        this.logger.warn(`H5 支付：用户 openid 为模拟值，已忽略: ${openid.substring(0, 20)}...`);
      }
    }
    ...
    try {
      if (isH5) {
        // H5 支付：无需 openid，微信返回 h5_url 供前端跳转唤起微信 App
        const result: any = await wxpay.transactions_h5({
          ...
          scene_info: {
            ...
            h5_info: {
              type: 'Wap',
              app_name: '社群名片',
              app_url: `https://${process.env.APP_DOMAIN || 'jugekeji.cn'}/mobile/order/pay/${orderNo}`,
            },
          },
        });

        this.logger.log(`微信 H5 支付下单返回: ${JSON.stringify(result)}`);
        return {
          channel: 'wechat_h5',
          h5Url: result.h5_url,
        };
      ...
      this.logger.log(`微信支付下单返回: ${JSON.stringify(result)}`);
      return {
        channel: 'wechat',
        appId: result.appId,
