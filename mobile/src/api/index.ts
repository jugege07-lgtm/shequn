...
export async function sendCode(phone: string, scene: 'login' | 'register' | 'reset_password' | 'bind_phone' = 'login') {
  return request.post('/api/auth/send-code', { phone, scene })
}
...
export async function createUnifiedOrder(orderId: number, payType?: 'jsapi' | 'h5') {
  return request.post('/api/pay/unified-order', { orderId, payType })
}
