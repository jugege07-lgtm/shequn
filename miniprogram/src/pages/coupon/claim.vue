  ...
  <div :style="sbStyle" class="phone-frame">
    ...
    <div class="coupon-grid">
      <div v-if="coupons.length === 0 && !loading" class="empty-tip">
        <image class="empty-icon" :src="iconEmptyDoc" mode="aspectFit" />
        <p>暂无可领优惠券</p>
        <span>敬请期待后续活动</span>
      </div>
        ...
        <div class="coupon-item-right">
          ...
          <div class="coupon-item-bottom">
            <div class="coupon-item-info">
              <span v-if="c.validFrom || c.validTo" class="info-time">
                <image class="info-time-icon" :src="iconClock" mode="aspectFit" />
                {{ formatRange(c) }}
              </span>
...
const iconEmptyDoc = svgUri(
  '<path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>',
  { color: '#999', strokeWidth: '1.5' }
)
const iconClock = svgUri('<circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>', { color: '#999' })
...
.empty-icon { width: 56px; height: 56px; opacity: 0.3; }
.empty-tip p { font-size: 14px; color: #666; }
.empty-tip text { font-size: 12px; color: #aaa; }
...
.coupon-item-info text { display: flex; align-items: center; gap: 3px; }
.info-time-icon { width: 11px; height: 11px; flex-shrink: 0; }
