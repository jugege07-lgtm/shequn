<template>
  <view :style="sbStyle" class="phone-frame dajia-page">
    <!-- Header -->
    <view class="header">
      <view class="header-left">
        ...
        <text class="header-title">大咖人脉</text>
      </view>
      <view class="header-vip-badge" v-if="isVip">
        <image :src="iconVipLayers" mode="aspectFit" />
        <text>VIP{{ minVipLevel }}</text>
      </view>
    </view>
    ...
    <view class="main-scroll">
      ...
      <!-- VIP 未开通 / 等级不足 -->
      <view class="vip-guard" v-else-if="!isVip">
        <view class="guard-icon">
          <image :src="iconGuardShield" mode="aspectFit" />
        </view>
        <view class="guard-title">专属大咖人脉</view>
        <view class="guard-desc">开通 VIP{{ minVipLevel }} 及以上会员，解锁大咖推荐、一键联系、人脉拓展等权益</view>
      ...
      <template v-else>
        <view class="intro-banner">
          <view class="intro-title">结识行业大咖，拓展优质人脉</view>
          <view class="intro-sub">点击「联系他」发起申请，对方同意后即可查看完整联系方式</view>
        </view>

        <view class="dajia-list" v-if="list.length">
          <view class="dajia-card" v-for="item in list" :key="item.id">
            <view class="dajia-head">
              <view class="dajia-avatar" :style="{ background: getAvatarColor(item.nickname || item.realName) }">
                <image v-if="avatarUrl(item.avatarUrl)" :src="avatarUrl(item.avatarUrl)" class="avatar-img" mode="aspectFill" />
                <text v-else>{{ (item.nickname || item.realName || '大咖').charAt(0) }}</text>
              </view>
              <view class="dajia-meta">
                <view class="dajia-name">
                  <text>{{ item.nickname || item.realName || '大咖' }}</text>
                  <view class="vip-tag" v-if="item.vipLevel"><image :src="iconVipTagLayers" mode="aspectFit" /><text>VIP{{ item.vipLevel }}</text></view>
                </view>
                <view class="dajia-title">{{ item.position || '行业精英' }}<text v-if="item.company"> · {{ item.company }}</text></view>
              </view>
            </view>

            <view class="dajia-intro" v-if="item.intro">{{ item.intro }}</view>

            <view class="dajia-phone" v-if="item.phone">
              <image :src="iconPhone" mode="aspectFit" />
              <text :class="{ unlocked: item.status === 'accepted' }">{{ item.phone }}</text>
              <text class="phone-tip" v-if="item.status !== 'accepted'">(加人脉后可见)</text>
            </view>

            <view class="dajia-actions">
              <button
              ...
              <button v-else-if="item.status === 'accepted'" class="connect-btn accepted" disabled>
                <image :src="iconCheck" mode="aspectFit" />
                已添加
              </button>
        ...
        <view class="empty-state" v-else>
          <image :src="iconEmptyUsers" mode="aspectFit" />
          <text>暂无大咖推荐，敬请期待</text>
        </view>
      </template>
    </view>
  </view>
...
const iconVipLayers = svgUri('<path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/>', { color: '#78350f' })
const iconGuardShield = svgUri('<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>', { color: '#f59e0b', strokeWidth: '1.5' })
const iconVipTagLayers = svgUri('<path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/>', { color: '#f59e0b' })
const iconPhone = svgUri('<path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.12.96.36 1.9.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.91.34 1.85.58 2.81.7A2 2 0 0122 16.92z"/>', { color: '#6366f1' })
const iconCheck = svgUri('<path d="M20 6L9 17l-5-5"/>', { color: '#10b981' })
const iconEmptyUsers = svgUri('<path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/>', { color: '#9ca3af', strokeWidth: '1.5' })
...
<style scoped>
.dajia-page { background: #f5f6fa; min-height: 100vh; }
...
.header-vip-badge image { width: 13px; height: 13px; }
...
.guard-icon image { width: 40px; height: 40px; }
...
.vip-tag image { width: 10px; height: 10px; }
...
.dajia-phone image { width: 16px; height: 16px; flex-shrink: 0; }
.dajia-phone text.unlocked { font-weight: 700; letter-spacing: 1px; color: #1e1b4b; }
.phone-tip { font-size: 12px; color: #9ca3af; }
.dajia-actions { display: flex; justify-content: flex-end; }
.connect-btn {
  display: inline-flex; align-items: center; gap: 4px;
  padding: 8px 22px; border-radius: 99px; font-size: 13px; font-weight: 600; border: none;
}
.connect-btn image { width: 14px; height: 14px; }
...
.connect-btn.requested { background: #f3f4f6; color: #6b7280; }
.connect-btn.accepted { background: rgba(16,185,129,0.1); color: #10b981; }
...
.empty-state image { width: 44px; height: 44px; margin-bottom: 10px; }
