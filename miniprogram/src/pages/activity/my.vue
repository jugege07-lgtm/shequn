<template>
  <div :style="sbStyle" class="phone-frame">
    ...
    <div class="main-scroll">
      ...
      <div class="list-card" v-else-if="filtered.length > 0">
        <div class="signup-item" v-for="s in filtered" :key="s.id" @click="$router.push('/activity/detail/' + s.id)">
          <div class="signup-cover" :style="{ background: s.cover }">
            <image v-if="s.coverImage && !s.coverError" class="signup-cover-img" :src="s.coverImage" mode="aspectFill" lazy-load @error="() => (s.coverError = true)" />
          </div>
...
// 图标（内联 svg → data URI；原 currentColor 按设计令牌取 #9ca3af）
const iconBack = svgUri('<path d="m15 18-6-6 6-6"/>', { color: '#1e1b4b' })
const iconCalendar = svgUri('<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>', { color: '#9ca3af', strokeWidth: '1.5' })
...
// toLocaleDateString 在小程序 iOS 上表现不稳定，手动格式化
function formatDateCN(input: string): string {
  if (!input) return '待定'
...
function mapActivity(item: any) {
  ...
  return {
    ...
    date: formatDateCN(item.startTime),
...
.signup-cover-img { width: 100%; height: 100%; }
...
.empty-icon image { width: 40px; height: 40px; }
