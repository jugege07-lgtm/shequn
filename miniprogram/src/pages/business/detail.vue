  ...
  <div class="phone-frame">
    <div class="header">
      ...
      <div class="header-right">
        <div class="header-icon" @click="openShare">
          <image :src="iconShare" mode="aspectFit" />
        </div>
        <div class="header-icon" @click="goHome">
          <image :src="iconHome" mode="aspectFit" />
        </div>
      </div>
    </div>
    <div class="main-scroll">
      ...
      <div class="biz-cover-hero" :style="{ background: coverBg }">
        <image
          v-if="rawBusiness?.coverImage && !coverError"
          :src="coverImageUrl"
          class="cover-img"
          mode="aspectFill"
      ...
      <div class="info-card contact-card">
        ...
        <template v-if="isUnlocked">
          <div class="contact-item" v-if="business.contactName">
            <image class="contact-icon" :src="iconContactUser" mode="aspectFit" />
            <span class="contact-label">联系人</span>
            <span class="contact-value">{{ business.contactName }}</span>
          </div>
          <div class="contact-item" v-if="business.phone">
            <image class="contact-icon" :src="iconContactPhone" mode="aspectFit" />
            <span class="contact-label">电话</span>
            <span class="contact-value contact-phone" @click="callPhone">{{ business.phone }}</span>
            <button class="contact-copy-btn" @click="copyPhone" aria-label="复制手机号">
              <image class="copy-icon" :src="iconCopy" mode="aspectFit" />
              <span>复制</span>
            </button>
          </div>
          <div class="contact-item" v-if="business.wechat">
            <image class="contact-icon" :src="iconContactWechat" mode="aspectFit" />
        ...
        <template v-else>
          ...
          <div class="unlock-prompt">
            <image class="prompt-icon" :src="iconLock" mode="aspectFit" />
          ...
          <div v-if="isFreeBiz && freeUnlock" class="free-unlock-tip">
            ...
            <span v-if="freeUnlock.remaining <= 0" class="upgrade-link" @click="$router.push('/vip/index')">升级VIP</span>
          </div>
          <!-- 免费次数耗尽，提示升级VIP -->
          <div v-if="isFreeBiz && freeUnlock && freeUnlock.remaining <= 0" class="vip-upgrade-banner">
            <span>本月免费解锁次数已用完</span>
            <span class="upgrade-btn" @click="$router.push('/vip/index')">立即升级VIP</span>
          </div>
        </template>
      </div>
    ...
    <div class="bottom-action" v-if="!isUnlocked">
      ...
      <div v-else-if="isFreeBiz && freeUnlock && freeUnlock.remaining <= 0" class="price-info-exhausted">
        ...
        <span class="exhausted-upgrade" @click="$router.push('/vip')">升级VIP解锁更多</span>
      </div>
      ...
      <button
        ...
        @click="handleUnlock"
      >
        <image v-if="!unlocking" class="btn-lock-icon" :src="iconLockWhite" mode="aspectFit" />
...
const iconShare = svgUri('<path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/>', { color: '#1e1b4b' })
const iconHome = svgUri('<path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><path d="M9 22V12h6v10"/>', { color: '#1e1b4b' })
const iconContactUser = svgUri('<path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/>', { color: '#6366f1' })
const iconContactPhone = svgUri('<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72"/>', { color: '#6366f1' })
const iconContactWechat = svgUri('<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/>', { color: '#6366f1' })
const iconCopy = svgUri('<rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/>', { color: '#6366f1' })
const iconLock = svgUri('<rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>', { color: '#6366f1' })
const iconLockWhite = svgUri('<rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>', { color: '#ffffff' })

// 返回兜底：冷启动（分享进入）历史栈为空时，shim 内部自动回退到首页
function goBack() {
  router.back()
}
...
// 点击电话拨号（小程序无 tel: 链接，改用 makePhoneCall）
function callPhone() {
...
.meta-item image { width: 14px; height: 14px; }
...
.contact-item .contact-icon { width: 18px; height: 18px; flex-shrink: 0; }
...
.contact-copy-btn::after { border: none; }
.contact-copy-btn .copy-icon { width: 14px; height: 14px; }
...
.unlock-prompt .prompt-icon { width: 16px; height: 16px; flex-shrink: 0; }
.unlock-prompt text { font-size: 12px; color: var(--color-text-tertiary); }
...
.unlock-btn .btn-lock-icon { flex-shrink: 0; width: 18px; height: 18px; }
