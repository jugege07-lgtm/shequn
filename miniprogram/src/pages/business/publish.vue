      ...
      <div class="form-card">
        ...
        <div class="form-group">
          ...
          <picker mode="selector" :range="categoryNames" @change="onCategoryChange">
            <div class="form-select" :class="{ 'placeholder': !form.categoryId }">
        ...
        <div class="form-group">
          ...
          <textarea v-model="form.description" class="form-textarea" placeholder="详细描述您的商机需求" :maxlength="500"></textarea>
        ...
        <div class="form-group">
          ...
          <div v-if="feeType === 'charge'" class="fee-input-row">
            <input v-model.number="form.unlockFee" type="digit" min="0" step="0.01" class="form-input fee-input" placeholder="请输入解锁费用" />
...
// picker 选择器数据：分类名称列表 + 当前选中名称
const categoryNames = computed(() => categories.value.map((c) => c.name))
const selectedCategoryName = computed(
  () => categories.value.find((c) => String(c.id) === String(form.categoryId))?.name || ''
)
function onCategoryChange(e: any) {
  const idx = Number(e?.detail?.value)
  const c = categories.value[idx]
  form.categoryId = c ? String(c.id) : ''
}
