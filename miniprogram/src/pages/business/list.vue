<template>
  <div class="page">
    ...
    <div class="header-bar">
      <div class="hero-card">
        ...
        <div class="search-box">
          <input v-model="searchKeyword" type="text" confirm-type="search" placeholder="搜索商机..." @input="debounceSearch" />
        </div>
      </div>
    ...
    <div v-else-if="filteredBusinesses.length === 0 && !loading" class="empty-tip">
      <image class="empty-icon" :src="iconEmpty" mode="aspectFit" />
      <p>{{ emptyText }}</p>
    ...
    <div class="list" v-else>
      <div class="item" v-for="b in filteredBusinesses" :key="b.id" @click="goDetail(b.id)">
        <div class="item-cover" :style="{ background: b.coverBg }">
          <image v-if="b.coverImage" :src="b.coverImageUrl" class="cover-img" mode="aspectFill" />
...
const iconUser = svgUri(
  '<path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/>',
  { color: '#9ca3af' }
)
const iconPlus = svgUri(
  '<path d="M12 5v14M5 12h14"/>',
  { color: '#ffffff', strokeWidth: '2.5' }
)
...
.empty-tip .empty-icon { width: 64px; height: 64px; opacity: 0.3; }
.empty-tip p { font-size: 16px; font-weight: 500; }
.empty-tip text { font-size: 13px; }
