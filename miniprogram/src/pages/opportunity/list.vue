<template>
  <div class="phone-frame">
    ...
    <div class="fab" @click="$router.push('/business/publish')">
      <image class="fab-icon" :src="iconPlus" mode="aspectFit" />
    </div>
  </div>
</template>
...
const iconUserSm = svgUri('<path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/>', { color: '#9ca3af' })
const iconCalendar = svgUri('<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>', { color: '#9ca3af' })
const iconClock = svgUri('<circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>', { color: '#9ca3af' })
const iconEye = svgUri('<path d="M2 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>', { color: '#9ca3af' })
const iconPlus = svgUri('<path d="M12 5v14M5 12h14"/>', { color: '#ffffff', strokeWidth: '2.5' })
...
// 价格 HTML（小程序版 sanitize 未导出 formatPriceHtml，此处为等价本地实现）
function formatPriceHtml(price: number, isFree: boolean): string {
  if (isFree) return '免费'
...
// 小程序无同源代理，统一用 normalizeImageUrl 补全为绝对地址
...
.meta-item .meta-icon { width: 14px; height: 14px; }
.opportunity-footer {
...
.opportunity-price text { font-size: 12px; font-weight: 400; }
...
.stat-item .stat-icon { width: 12px; height: 12px; }
...
.load-more-btn::after { border: none; }
...
.fab .fab-icon { width: 26px; height: 26px; }
.fab:active {
